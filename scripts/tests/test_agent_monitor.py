...
class TestAdaptiveIC:
    def test_empty_registry_returns_floor(self, daemon, registry_path):
        """No registry file → use config floor (0.10)."""
        ic = daemon._compute_adaptive_ic()
        assert ic == 0.10

    def test_empty_registry_file_returns_floor(self, daemon, registry_path):
        """Empty registry list → use floor."""
        _write_registry(registry_path, [])
        ic = daemon._compute_adaptive_ic()
        assert ic == 0.10

    def test_single_signal(self, daemon, registry_path):
        """One signal with IC=0.50 → max(0.10, 0.50*0.8) = 0.40."""
        _write_registry(registry_path, [_make_signal(ic=0.50)])
        ic = daemon._compute_adaptive_ic()
        assert ic == pytest.approx(0.40)

    def test_multiple_signals_uses_median(self, daemon, registry_path):
    ...
    def test_high_quality_registry_raises_bar(self, daemon, registry_path):
    ...
    def test_low_quality_registry_uses_floor(self, daemon, registry_path):
    ...
    def test_retired_signals_excluded(self, daemon, registry_path):
    ...
    def test_even_count_median(self, daemon, registry_path):
...
class TestICDecayDetection:
    ...
    def test_healthy_signal_no_decay(self, daemon, registry_path):
        """IC above threshold → decay_days stays at 0."""
        sig = _make_signal(ic=0.50)
        _write_registry(registry_path, [sig])
    ...
    def test_decaying_signal_increments_counter(self, daemon, registry_path):
    ...
    def test_cumulative_decay_days(self, daemon, registry_path):
    ...
    def test_decay_counter_resets_on_recovery(self, daemon, registry_path):
    ...
    def test_no_data_skips_decay_check(self, daemon, registry_path):
        ...
        with open(registry_path) as f:
            reg = json.load(f)
        assert "decay_days" not in reg[0]  # never set


class TestAutoRetirement:
    """Test that signals are auto-retired after sustained IC decay."""

    def test_retirement_at_limit(self, daemon, registry_path):
    ...
    def test_not_retired_before_limit(self, daemon, registry_path):
    ...
    def test_retirement_updates_signal_count(self, daemon, registry_path):
    ...
    def test_retired_signals_skipped_by_monitor(self, daemon, registry_path):
    ...
    def test_multiple_signals_independent(self, daemon, registry_path):
    ...
    def test_custom_decay_config(self, daemon, registry_path):
...
class TestICHistory:
    ...
    def test_history_appended(self, daemon, registry_path):
    ...
    def test_history_bounded_at_30(self, daemon, registry_path):
...
class TestPromotionCheck:
    ...
    def test_promotable_signal(self, daemon, registry_path):
    ...
    def test_not_promotable_low_sharpe(self, daemon, registry_path):
