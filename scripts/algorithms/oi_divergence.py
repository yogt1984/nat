...
class OIDivergence(MicrostructureAlgorithm):
    ...
    def step(self, tick: dict[str, float]) -> dict[str, float]:
        ...
        # Divergence: price going up but OI shrinking (or vice versa) = weak move
        # Normalize both to comparable scale
        price_sign = np.sign(price_trend)
        oi_sign = np.sign(oi_trend)

        divergence = price_trend * 1000 - oi_trend * 100  # scale-adjusted
    ...
    def run_batch(self, df: "pd.DataFrame") -> "pd.DataFrame":
        ...
        mid_s = pd.Series(mid)
        price_trend = np.log(mid_s / mid_s.shift(self._price_window)).values

        # Rolling OI trend (relative change)
        oi_s = pd.Series(oi)
        oi_start = oi_s.shift(self._oi_window)
        oi_trend = ((oi_s - oi_start) / (oi_start + 1e-12)).values

        divergence = price_trend * 1000 - oi_trend * 100

        price_sign = np.sign(price_trend)
        oi_sign = np.sign(oi_trend)
        confirmation = ((price_sign == oi_sign) & (price_sign != 0)).astype(np.float64)
        confirmation[np.isnan(price_trend) | np.isnan(oi_trend)] = np.nan
