...
| COST-5 | Quantify the maker *volume*-tier assumption | TODO | P1 | S | in-hand | X-1 | SSOT prices a 0.2 bps maker **rebate** (presumes an untested volume tier); venue base perp maker rate is a **+1.5 bps fee**. At base rate §4.7's +0.031 bps EV is deeply negative. Add `[hyperliquid_maker_tiers]` + reprice — the real fee risk (FINDINGS §4.10 closing note). |
