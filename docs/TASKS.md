...
10. `A-2` — combiner revalidation. §5's IC .18/.25/.36 rests on a **2-day** OOS with monotonically
   rising fold ICs; it is the last unrefuted capital-relevant claim in the record. Run it before
   anything is built on top of it.
11. `A-1` / `A-3` — agreement-gate standing eval; conditional-IC surface refresh.
...
| HYG-8 | `requirements.lock` does not describe what the code imports | TODO | **P1** | S | in-hand | — | **Found 2026-08-07 building the first venv on su-75 from `requirements.lock`** — the lock is not reproducible. **(a) `toml`:** ~10 modules import it — `config_utils.py`, `risk/kill_switch.py`, `promotion_daemon.py`, `it_engine/config.py`, `it_engine/null_calibration.py` (PROC-12) — but the only related pin is `tomli>=2.0.0; python_version < "3.11"`, a **no-op on 3.12**, so a clean install cannot import the kill switch or the null-calibration layer. Python 3.11+ ships `tomllib` in the stdlib, so the right fix is likely migrating readers to `tomllib` rather than pinning the unmaintained `toml`. **(b) `optuna`:** imported by `swarm/optuna_optimizer.py`, absent from the lock — the documented `nat evolve` Tier-3 feature cannot run from a clean install. Both surface as collection errors (`test_tier2_swarm.py`, `test_tier3_optuna.py`). CI does not `--ignore` either file, so **CI is either failing on master or its runner has these transitively** — worth confirming, because a lock that only works on machines that already have the packages is not a lock. |
| HYG-9 | **The suite is red on master** — 16 failed / 38 errors | TODO | **P1** | M | in-hand | HYG-8 | **Measured 2026-08-07** on `ba0a253` in a clean venv: `4347 passed, 16 failed, 38 errors` (13 m 48 s). Verified pre-existing — the two plausibly-mine failures reproduce byte-identically with my branch stashed. **CI's `--ignore` list covers none of these files, so CI is failing on master.** Clusters: (a) `test_agent_monitor` 28 + `test_agent_ensemble` 10 errors — fixture/refactor drift (`module 'agent.daemon' has no attribute 'ROOT'`, `'NoneType' has no attribute 'load_registry'`); (b) `test_commands_snapshot` — command count 331 → **333**, i.e. the upstream XS-1 merge added CLI surface without refreshing the baseline (a same-branch-status-rule miss in the test dimension); (c) `test_help_coverage` — 4 groups (`ing`, `metrics`, `package`, `service`) lack curated `nat help` entries; (d) `test_daemon_integration` 5, `test_macro_agent` 3, `test_alpha_pipeline` 3, `test_mf_agent` 2. **(e) `test_algorithm_smoke`: `microprice: first 50 (warmup) rows not NaN-blanked` — a genuine `contracts/algorithm.md` violation in HF1**, the maker-line anchor merged 2026-07-30, and the one item here that touches research correctness rather than plumbing. Triage (e) first. |
