...
| VW-1 | `vwap_multiscale` offline transform | TODO | **P1** | S | in-hand | — | Step A1 of `specs/multiscale_vwap.md`. 1-min bucketed accumulators over `data/trades/` (**44 d, 1,193 files, 203 MB** already on disk) → `vwap_{w}` / `vwap_dev_{w}` / `vwap_dev_z_{w}` for w ∈ 5m,10m,15m,1h,6h,12h. ~12 KB/symbol vs ~17 MB of raw trades. **New columns use the intuitive `(price−vwap)/vwap`; the shipped `flow_vwap_deviation` is inverted and stays that way** (re-signing it would invalidate §1's IC record). |
| VW-2 | Multi-scale VWAP study | TODO | **P1** | M | in-hand | VW-1 | Step A2. PROC-4 durability + PROC-20 band structure + PROC-15 redundancy per window, criteria committed **before** the run. A window earns a column only if informative (z≥3), day-durable (≥0.55), FDR-surviving, **non-redundant vs the faster window**, and with an event rate that makes a verdict reachable (PROC-20's ~1.5/day is the counter-example). |
| VW-3 | Feature-vector migration (schema change) | BLOCKED | P2 | M | in-hand | VW-2 | Step B — **only for windows that pass VW-2; if none pass this closes with a FINDINGS entry and the ingestor is untouched.** Guardrail: ripple analysis before code (236→248 max), readers must tolerate absent columns (44 d of existing parquet lack them). Rust `VwapRing` of 1-min buckets — **not** a widened `TradeBuffer`: six O(n) `trades_in_window` scans per symbol at 10 Hz would blow the 80 ms/tick p99 budget. |
