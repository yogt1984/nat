...
| ID | Title | Status | Prio | Eff | Data | Dep | Notes |
|----|-------|--------|------|-----|------|-----|-------|
| XS-1 | Universe candle backfill | TODO | **P0** | S | in-hand | — | **The unblocker for `B-5` and all of Track C.** Extend `data/fetch_candles.py` (today: explicit `--symbol` list, no enumeration) with a `--universe` flag over `HyperliquidClient.get_meta()`, which **already parses `universe`** (`:169-186`) — wiring, not new API work. 1 m + 1 h × ~150 perps × ≥90 d → `data/candles/`; coverage report enumerating gaps (never silently dropping them); incremental daily cron. |
