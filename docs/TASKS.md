...
| B-5a | Wide-pair breakeven **arithmetic screen** | TODO | **P1** | S | in-hand | XS-8 | The cheap kill-test, and it needs no simulation. Rank the 177 pairs by measured half-spread (`XS-8`), then apply §4.11's relation `breakeven = E[adverse\|fill] − half_spread`. **XS-8's first sweep sharpened the question (FINDINGS §7.2): universe median half-spread is 1.372 bps, 17.7× BTC, with 169/177 pairs wider than BTC and SOL in the tightest five** — the sampling bias §4.11 suspected is now measured. So the screen no longer asks *is any pair wider* (yes, nearly all) but **the two questions that actually decide it**: (1) does `E[adverse\|fill]` scale with the half-spread? It is measured only for BTC/ETH/SOL, and if it scales proportionally the ratio is unchanged and nothing improves — bound it and **state the bound, do not bury it**; (2) is there any size there? The widest pairs are nearly empty at the touch (XAI 12.9 bps on **$20**, HMSTR 26.8 bps on $3.3 k), so the joint wide-AND-deep requirement is far tighter than either margin — `XS-5`'s admission test. Wait for several days of `XS-8` samples first: one sweep is n=1. |
