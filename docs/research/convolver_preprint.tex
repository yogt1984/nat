...
shapes are temporally persistent ($\bar{\rho} > 0.90$ for the strongest
kernels), supporting their use as real-time features via cosine similarity
scoring. In walk-forward validation, however, no kernel yet meets the
out-of-sample robustness threshold and several flip sign out of sample:
at the current data volume ($\sim$50--70 test-set events per type) the
discovered shapes are candidate features, not validated signals, and we
estimate 3--6$\times$ more data is required for a decisive test.
\end{abstract}
...
The application of principal component analysis (Jolliffe, 2002) and
eigenvalue decomposition to financial correlation matrices is well
established (Laloux \emph{et al.}, 1999; Plerou \emph{et
...
onto shape $k$. The decomposition is computed using standard numerical SVD
algorithms (Golub and Van Loan, 2013).
...
By the Eckart--Young theorem (Eckart and Young, 1936), the retained
...
\item \textbf{Multi-channel contribution.} The 6 survivors span all four
      channels (body, upper wick, lower wick, volume), confirming that the
...
and several kernels flip sign out of sample.
The event counts are consistent with a data limitation rather than a
methodological failure---in-sample ICs are strong ($|\mathrm{IC}|$ up to
0.47) and turtle-type kernels show partial OOS generalisation with
consistent sign---but until a decisive test is run, the honest status of
every discovered kernel is \emph{candidate}, not \emph{validated}. Future work will extend the analysis to
